...
const WINDOW_SCENARIOS: WindowScenario[] = [
  {
    ...
    iconBg: 'bg-indigo-600/8 border border-indigo-600/10',
    recommendation: 'Night Release',
    reasons: ['Minimise customer exposure', 'Reduce potential business impact'],
    tag: 'Night',
    tagColor: 'text-indigo-600 bg-indigo-600/8 border-indigo-600/15',
  },
  {
    scenario: 'Low Traffic + E2E Testable',
    icon: Sun,
    iconColor: 'text-confidence-conditional',
    iconBg: 'bg-confidence-conditional/8 border border-confidence-conditional/10',
    recommendation: 'Day Release',
    reasons: ['Safe validation through available test profiles', 'Faster issue identification'],
    tag: 'Day',
    tagColor: 'text-confidence-conditional bg-confidence-conditional/8 border-confidence-conditional/15',
  },
  {
...
const SCORING_DRIVERS: ScoringDriver[] = [
  { risk: 'Missing TCC Approval', category: 'Governance Risk', icon: ShieldCheck, color: 'text-indigo-600' },
  { risk: 'Insufficient Namespace Capacity', category: 'Deployment Risk', icon: Server, color: 'text-cyan-600' },
  { risk: 'Missing/Failed Performance Tests', category: 'Quality Risk', icon: Gauge, color: 'text-confidence-conditional' },
  { risk: 'High Error Rate or E95 Regression', category: 'Customer Experience Risk', icon: Activity, color: 'text-hsbc-red' },
  { risk: 'Documentation Non-Compliance', category: 'Operational Risk', icon: FileText, color: 'text-pink-600' },
  { risk: 'High-Risk Code Delta', category: 'Change Risk', icon: Code, color: 'text-hsbc-red' },
];
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-8 py-8 h-full flex flex-col">
        ...
        <div className="animate-fade-in mb-5">
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 rounded-xl bg-hsbc-red/8 border border-hsbc-red/15 flex items-center justify-center">
              <ToggleLeft size={20} className="text-hsbc-red" />
            </div>
            <h2 className="text-4xl font-bold text-hsbc-gray-900">How It Works</h2>
          </div>
          ...
          <div className="w-[42%] flex flex-col gap-4 min-h-0 overflow-y-auto pr-1">
            ...
            <div className="glass-card p-4 animate-fade-in-left stagger-1">
              <div className="flex items-center gap-2 mb-3">
                <div className="w-6 h-6 rounded-md bg-hsbc-red/8 border border-hsbc-red/15 flex items-center justify-center">
                  <span className="text-[10px] font-bold text-hsbc-red">A</span>
                </div>
                <h3 className="text-sm font-semibold text-hsbc-gray-800">Release Window Decisioning</h3>
              </div>
              <div className="flex flex-col gap-2">
                {WINDOW_SCENARIOS.map((s, i) => (
                  <div key={i} className="bg-hsbc-offwhite rounded-xl px-3 py-2.5">
                    <div className="flex items-center gap-2 mb-1.5">
                      <div className={`p-1 rounded-md ${s.iconBg}`}>
                        <DynamicIcon icon={s.icon} size={14} className={s.iconColor} />
                      </div>
                      <span className="text-xs font-semibold text-hsbc-gray-700 flex-1">{s.scenario}</span>
                    ...
                    <div className="ml-5 flex flex-col gap-0.5">
                      {s.reasons.map((r, j) => (
                        <div key={j} className="flex items-center gap-1.5">
                          <div className="w-1 h-1 rounded-full bg-hsbc-gray-300" />
            ...
            <div className="glass-card p-4 animate-fade-in-left stagger-3">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-semibold text-hsbc-gray-800">Explainable Confidence Scoring</h3>
              </div>

              {/* Scoring Bands Visual */}
              <div className="flex gap-2 mb-3">
                <div className="flex-1 bg-confidence-go/5 border border-confidence-go/10 rounded-lg px-3 py-2 text-center">
                  ...
                  <div className="text-[10px] text-hsbc-gray-500">Go</div>
                </div>
                <div className="flex-1 bg-confidence-conditional/5 border border-confidence-conditional/10 rounded-lg px-3 py-2 text-center">
                  ...
                  <div className="text-[10px] text-hsbc-gray-500">Conditional Go</div>
                </div>
                <div className="flex-1 bg-confidence-nogo/5 border border-confidence-nogo/10 rounded-lg px-3 py-2 text-center">
                  <XCircle size={16} className="text-confidence-nogo mx-auto mb-0.5" />
                  <div className="text-sm font-bold text-confidence-nogo">&lt;50%</div>
                  <div className="text-[10px] text-hsbc-gray-500">No-Go</div>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                {SCORING_DRIVERS.map((driver) => (
                  <div key={driver.risk} className="flex items-center gap-2 bg-hsbc-offwhite rounded-lg px-3 py-1.5">
                    <DynamicIcon icon={driver.icon} size={12} className={driver.color} />
                    <span className="text-[11px] text-hsbc-gray-700 flex-1">{driver.risk}</span>
                    <span className="text-[9px] text-hsbc-gray-400 bg-white px-1.5 py-0.5 rounded border border-hsbc-gray-100">{driver.category}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="w-[58%] flex flex-col min-h-0">
            <div className="glass-card p-4 flex-1 flex flex-col min-h-0 animate-fade-in-right stagger-2">
              <div className="flex items-center gap-2 mb-3 shrink-0">
                ...
                <h3 className="text-sm font-semibold text-hsbc-gray-800">Edge Cases ReleaseRadar Explicitly Handles</h3>
              </div>
              <div className="flex-1 overflow-y-auto pr-1">
                <div className="grid grid-cols-1 gap-2">
                  {EDGE_CASES.map((ec, i) => (
                    <div
                      key={i}
                      className={`bg-hsbc-offwhite rounded-xl px-4 py-2.5 border-l-2 ${
                        ec.severity === 'nogo'
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        />
                        <span className="text-xs font-semibold text-hsbc-gray-800">{ec.title}</span>
                        <span className={`ml-auto text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-full ${
                          ec.severity === 'nogo'
                            ? 'bg-confidence-nogo/5 text-confidence-nogo border border-confidence-nogo/10'
                      ...
                      <div className="ml-5 flex flex-wrap gap-1">
                        {ec.actions.map((action, j) => (
                          <span key={j} className="text-[10px] text-hsbc-gray-500 bg-white px-2 py-0.5 rounded border border-hsbc-gray-100">
                            {action}
                          </span>
                        ))}
                      </div>
